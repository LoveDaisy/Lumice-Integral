...
figure drops the first ``--warmup`` pixels (cold kernels, first prescan
...
Usage (Mac laptop, a few minutes with a 4M-sample prescan built in memory):
    ...
    uv run python benchmarks/benchmark_column_steady_state.py --column 126 --rows 0:801 \\
        --prescan-cache /tmp/prescan.npz --output /tmp/column-126.json
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--prescan-cache", type=Path, default=None, help="optional .npz cache of the prescan table")
    ...
    from lumice_integral.strip_driver import DriverOptions, PrescanBuildOptions, build_scene_prescan_table, render_column
    ...
    options = DriverOptions(prescan=PrescanBuildOptions(cache_path=args.prescan_cache))
