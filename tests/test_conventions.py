...
def test_phi_key_indexes_the_d6h_tuple_by_matrix_not_by_published_number(faces):
    """The interface for task 19: ``phi_key[0]`` is an index into :func:`hexprism_symmetry_matrices` (closure order).

    Only the matrix is meaningful across projects (the writing series'
    ``reflection_group.identify(M)`` gives its #1-#12); the index is not a
    published number.  Every prism fold matrix is one of the 24 and one of
    the 12 of the group ``G`` generated by the four mirrors (``M^2`` has
    order dividing 3 and ``det M = +-1``).
    """
