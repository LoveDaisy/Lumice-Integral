...
| 6 | Phase II base point `u` | — | theorem 8: `u = R⁻¹s` (the sun in the crystal frame) | `u = R⁻¹ŝ`: `s2_store` events (`SCHEMA_VERSION = 3`), `align_rotations(u, sun)`: `R u = ŝ`; roadmap §4 | `test_store_u_phi_and_d_are_framework_theorem_8`; `tests/test_s2_store.py::test_events_match_the_batch_evaluators_directly` |
...
**Phase II store schema.** `s2_store.SCHEMA_VERSION = 3` records `u = R⁻¹ŝ`
