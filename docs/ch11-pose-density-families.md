...
19 profile rows; measured 2026-09-20 with the then-default 400k-sample prescan, which the script has since replaced by the shared S² store seeds of task `phase1-seeds-from-store`). Families at the
...
Haar samples of `3-5` (64k valid of 400k in the 2026-09-20 run, then drawn from the retired prescan table) weighted by each
