...
- **候选**来自一张场景级预扫表（`prescan.PrescanTable`：$4\times10^6$ 个 Haar 姿态，把域内有效的按出射方向建索引），而不是逐像素预扫；
...
- **重采样后固定网格求积**：对曲线做样条重采样，批量回投、批量求因子，Simpson 以 $N$ 对 $N/2$ 作误差估计；
...
**绝对尺度。** 不拟合任何参数，本渲染器与 Lumice 在匹配折射率下的亮带上一致到 `0.997-0.999`，中间经过一个逐像素因子 $K_p$。推导它时暴露出 Lumice 给每个姿态分配相等能量，而不是按晶体投影面积加权；作者裁定这是 Lumice 的 bug 并在那边修复（Ice Halo #597；待重新核对，任务 `lumice-area-weighting-recheck`）。
...
已记录、不阻塞的开放项：完整性证书（由 Phase II 的临界点提供，[phase2_zh.md](phase2_zh.md) §3.1）、有限日盘、焦散带的像素平均、契约 §12 的显式事件定位。计划中的改动：Phase I 的 seed 改由 $S^2$ 事件仓库而非预扫表提供，并附带统计性的完整性交叉检查（phase2_zh.md §6；M2 子任务 `phase1-seeds-from-store`）。
