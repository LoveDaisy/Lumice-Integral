...
**边界不需要事件处理。** 被积函数在每条边界上都连续地归零：走廊边界（两个多边形分离，$A_P$ 连续趋于 0）；公式域 $U_P$ 的出射面 TIR 边界（Fresnel 透过率在临界角处趋于 0）；入射没有临界角；内部反射的部分反射是连续权重，不是边界：$T_P$ 等于入射与出射透过率乘以每次内反射的反射率 $R_k$（TIR 时 $R_k = 1$，在临界角处连续），即 Lumice 的逐界面非偏振分裂（[conventions.md](conventions.md) #18）。任务 `optics-partial-reflection`（2026-09-25）起已实现（此前代码只承认内部全反射，部分反射直接把光路判为不可行，见 §4 的 Liljequist fixture 与 [roadmap.md](roadmap.md) §9，2026-09-25）。所有读取 `optics` 门的地方（$S^2$ 仓库、`weights`、Phase I 追踪器的定义域判定）都已看到放宽后的定义域；任务 `dp-field-partial-reflection-boundaries` 起 $D_P$ 场层与等值线走线器也是（§3.1）：内部 TIR 判别式不是 $\partial U_P$ 的 margin。连续不等于光滑：起点处 $1 - R_k \sim \sqrt{-\mathrm{disc}}$，所以在 $D_P$ 的水平集离开某条起点曲线 $\{\mathrm{disc}_k = 0\}$ 的地方（该曲线上 $D$ 的极值），剖面数值连续、斜率跳变（$\int\sqrt{c - bx^2}\,dx \propto c$）：这是一个拐角，不是 $D_P$ 的临界值。Liljequist 峰就是这样一个拐角（§4；任务 `ch10-liljequist-unblock-and-docs`）。Phase I 在含内反射光路上尚未交叉验证，那是它自己的任务（roadmap §0）。被 $\partial V_P$ 截断的等值线在 $U_P$ 上照常追踪，由 $A_P T_P$ 去掉不可行的部分；Phase I 的规则「继续追踪、权重归零」（契约 §6.3）就是把同一个事实放进了追踪器里。剩下的唯一非光滑性是 $A_P$ 的折点（某个顶点越过某条棱），它和 Phase I 一样会让局部求积降阶。
...
- **Liljequist 与 142° 幻日**（写作第 8、10 章）。*已裁定（任务 `ch10-liljequist-unblock-and-docs`，`lumice_integral.ch10_verdicts.liljequist`；§4 与英文版附录「Chapter-10 verdicts rerun」）：*142° 边缘是 A60-10 的鞍点 $120° + D_{\min}(\texttt{3-5}) = 141.839300°$，生产链路实测，与形状无关；`3-5-6-7-3` 的 Liljequist 峰对每个 $h/a$ 都在 153.07°，是剖面在 $R_k$ 内部 TIR 起点处的拐角（不是 $D_P$ 的临界值），峰宽由窗口决定。两层结构在 Lumice 未过滤的 ch8 条带里出现在同样的方位角。测试：`tests/test_ch10_verdicts.py`（`test_a60_10_*`、`test_liljequist_*`）。
...
| 第 10 章裁定（内缘、Liljequist、幻日环、聚光标签） | 已实测；部分内反射下重跑（A60-10 鞍点 141.839300°、Liljequist 峰为 153.07° 的 TIR 起点拐角、`1-3-2` 窗口无跳变、`1-3-5-2` 在 120° 有 Jacobian 聚光） | §10、英文版附录；任务 `ch10-numerical-verdicts`、`ch10-liljequist-unblock-and-docs` |
| 发散光 | 推导 | backlog |
